...
class SystemDesignTest {
    ...
    companion object {
        fun createSampleDesign() = SystemDesign(
            title = "URL Shortener",
            prompt = "Design a URL shortener",
            assumptions = listOf("Global service", "Hundreds of millions of links"),
            coreConcept = "A URL shortener is a mapping system: it stores a long URL under a short code and redirects users from the short code back to the original URL.",
            requirements = Requirements(
            ...
            highLevelArchitecture = DesignSection(
                title = "Architecture Overview",
                content = "The system uses a simple three-tier architecture with a CDN for caching redirects, an API service for creating and resolving short links, and a database for persistent storage."
            ),
            deepDives = listOf(
                DesignSection(
                    title = "ID Generation",
                    content = "We use Base62-encoded random IDs (7 characters) giving ~3.5 trillion unique codes. Collisions are handled by retry."
                )
            ),
            failureModes = listOf(
